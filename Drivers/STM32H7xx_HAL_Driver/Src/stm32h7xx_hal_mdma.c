 ...
 */
static void
MDMA_SetConfig(MDMA_HandleTypeDef *hmdma, uint32_t SrcAddress,
		uint32_t DstAddress, uint32_t BlockDataLength, uint32_t BlockCount);
static void
MDMA_Init(MDMA_HandleTypeDef *hmdma);
...
HAL_StatusTypeDef HAL_MDMA_Init(MDMA_HandleTypeDef *hmdma)
{
	...
	assert_param(
			IS_MDMA_BUFFER_TRANSFER_LENGTH(hmdma->Init.BufferTransferLength));
	assert_param(
			IS_MDMA_TRANSFER_TRIGGER_MODE(hmdma->Init.TransferTriggerMode));
	assert_param(
			IS_MDMA_BLOCK_ADDR_OFFSET(hmdma->Init.SourceBlockAddressOffset));
...
HAL_StatusTypeDef HAL_MDMA_ConfigPostRequestMask(MDMA_HandleTypeDef *hmdma,
		uint32_t MaskAddress, uint32_t MaskData)
{
...
HAL_StatusTypeDef HAL_MDMA_RegisterCallback(MDMA_HandleTypeDef *hmdma,
		...
		void (*pCallback)(MDMA_HandleTypeDef *_hmdma))
{
	HAL_StatusTypeDef status = HAL_OK;
		...
		MDMA_LinkNodeConfTypeDef *pNodeConfig)
{
	...
	assert_param(
			IS_MDMA_BUFFER_TRANSFER_LENGTH(pNodeConfig->Init.BufferTransferLength));
	assert_param(
			IS_MDMA_TRANSFER_TRIGGER_MODE(pNodeConfig->Init.TransferTriggerMode));
	assert_param(
			IS_MDMA_BLOCK_ADDR_OFFSET(pNodeConfig->Init.SourceBlockAddressOffset));
	assert_param(
			IS_MDMA_BLOCK_ADDR_OFFSET(pNodeConfig->Init.DestBlockAddressOffset));
	...
	pNode->CTCR = pNodeConfig->Init.SourceInc | pNodeConfig->Init.DestinationInc
			...
			| ((pNodeConfig->Init.BufferTransferLength - 1U)
					<< MDMA_CTCR_TLEN_Pos)
	 ...
	 */
	if ((pNodeConfig->Init.Request == MDMA_REQUEST_SW)
			|| (pNodeConfig->PostRequestMaskAddress != 0U))
	{
		pNode->CTCR |= MDMA_CTCR_BWM;
	}

	/* Set the new CBNDTR Register value */
	pNode->CBNDTR = ((pNodeConfig->BlockCount - 1U) << MDMA_CBNDTR_BRC_Pos)
			& MDMA_CBNDTR_BRC;
	...
	{
		/*write new CBRUR Register value : source repeat block offset */
		pNode->CBRUR = (((uint32_t) pNodeConfig->Init.SourceBlockAddressOffset)
				& 0x0000FFFFU);
	}
	...
	{
		/*write new CBRUR Register value : destination repeat block offset */
		pNode->CBRUR |= ((((uint32_t) pNodeConfig->Init.DestBlockAddressOffset)
				& 0x0000FFFFU) << MDMA_CBRUR_DUV_Pos);
	}
...
HAL_StatusTypeDef HAL_MDMA_LinkedList_AddNode(MDMA_HandleTypeDef *hmdma,
		MDMA_LinkNodeTypeDef *pNewNode, const MDMA_LinkNodeTypeDef *pPrevNode)
{
	...
	if (HAL_MDMA_STATE_READY == hmdma->State)
	{
		...
		else if (hmdma->FirstLinkedListNodeAddress != pNewNode)
		{
			...
			while ((counter < hmdma->LinkedListNodeCounter)
					&& (hal_status == HAL_OK))
			{
			...
			if (hal_status == HAL_OK)
			{
				...
				if ((pPrevNode == hmdma->LastLinkedListNodeAddress)
						|| (pPrevNode == NULL))
				{
					/* insert the new node at the end of the list */
					pNewNode->CLAR = hmdma->LastLinkedListNodeAddress->CLAR;
					hmdma->LastLinkedListNodeAddress->CLAR =
							(uint32_t) pNewNode;
					/* Update the Handle last node address */
					hmdma->LastLinkedListNodeAddress = pNewNode;
		...
		MDMA_LinkNodeTypeDef *pNode)
{
	...
	if (HAL_MDMA_STATE_READY == hmdma->State)
	{
		...
		if (((uint32_t) hmdma->FirstLinkedListNodeAddress == 0U)
				|| ((uint32_t) hmdma->LastLinkedListNodeAddress == 0U)
				|| (hmdma->LinkedListNodeCounter == 0U))
		{
			hal_status = HAL_ERROR;
		}
		else if (hmdma->FirstLinkedListNodeAddress == pNode) /* Deleting first node */
		{
			...
			{
				if ((uint32_t) hmdma->FirstLinkedListNodeAddress
						== hmdma->LastLinkedListNodeAddress->CLAR)
				{
				...
				hmdma->FirstLinkedListNodeAddress =
						(MDMA_LinkNodeTypeDef*) hmdma->Instance->CLAR;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_MDMA_LinkedList_EnableCircularMode(
		MDMA_HandleTypeDef *hmdma)
{
	...
	if (HAL_MDMA_STATE_READY == hmdma->State)
	{
		...
		else
		{
			...
			hmdma->LastLinkedListNodeAddress->CLAR =
					(uint32_t) hmdma->FirstLinkedListNodeAddress;
		}
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_MDMA_LinkedList_DisableCircularMode(
		MDMA_HandleTypeDef *hmdma)
{
	HAL_StatusTypeDef hal_status = HAL_OK;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_MDMA_Start(MDMA_HandleTypeDef *hmdma, uint32_t SrcAddress,
		uint32_t DstAddress, uint32_t BlockDataLength, uint32_t BlockCount)
{
	/* Check the parameters */
	...
	if (HAL_MDMA_STATE_READY == hmdma->State)
	{
		...
		MDMA_SetConfig(hmdma, SrcAddress, DstAddress, BlockDataLength,
				BlockCount);
...
HAL_StatusTypeDef HAL_MDMA_Start_IT(MDMA_HandleTypeDef *hmdma,
		uint32_t SrcAddress, uint32_t DstAddress, uint32_t BlockDataLength,
		uint32_t BlockCount)
{
	/* Check the parameters */
	assert_param(IS_MDMA_TRANSFER_LENGTH(BlockDataLength));
...
HAL_StatusTypeDef HAL_MDMA_Abort(MDMA_HandleTypeDef *hmdma)
{
	...
	else
	{
		...
		__HAL_MDMA_DISABLE_IT(hmdma,
				(MDMA_IT_TE | MDMA_IT_CTC | MDMA_IT_BT | MDMA_IT_BRT | MDMA_IT_BFTC));
		...
		__HAL_MDMA_CLEAR_FLAG(hmdma,
				(MDMA_FLAG_TE | MDMA_FLAG_CTC | MDMA_FLAG_BT | MDMA_FLAG_BRT | MDMA_FLAG_BFTC));
...
HAL_StatusTypeDef HAL_MDMA_PollForTransfer(MDMA_HandleTypeDef *hmdma,
		HAL_MDMA_LevelCompleteTypeDef CompleteLevel, uint32_t Timeout)
{
	...
	levelFlag = (
			(CompleteLevel == HAL_MDMA_FULL_TRANSFER) ? MDMA_FLAG_CTC :
	...
	{
		__HAL_MDMA_CLEAR_FLAG(hmdma,
				(MDMA_FLAG_BFTC | MDMA_FLAG_BT | MDMA_FLAG_BRT));
	}
	else if (CompleteLevel == HAL_MDMA_FULL_TRANSFER)
	{
		__HAL_MDMA_CLEAR_FLAG(hmdma,
				(MDMA_FLAG_BRT | MDMA_FLAG_BT | MDMA_FLAG_BFTC | MDMA_FLAG_CTC));
...
HAL_StatusTypeDef HAL_MDMA_GenerateSWRequest(MDMA_HandleTypeDef *hmdma)
{
	...
	else if (((hmdma->Instance->CISR & MDMA_CISR_CRQA) != 0U)
			|| (request_mode == 0U))
	{
...
void HAL_MDMA_IRQHandler(MDMA_HandleTypeDef *hmdma)
{
	...
	generalIntFlag = 1UL
			<< ((((uint32_t) hmdma->Instance - (uint32_t) (MDMA_Channel0))
					/ HAL_MDMA_CHANNEL_SIZE) & 0x1FU);
 ...
 * @retval HAL status
 */
static void MDMA_SetConfig(MDMA_HandleTypeDef *hmdma, uint32_t SrcAddress,
		uint32_t DstAddress, uint32_t BlockDataLength, uint32_t BlockCount)
{
	uint32_t addressMask;

	/* Configure the MDMA Channel data length */
	MODIFY_REG(hmdma->Instance->CBNDTR, MDMA_CBNDTR_BNDT,
			(BlockDataLength & MDMA_CBNDTR_BNDT));

	/* Configure the MDMA block repeat count */
	MODIFY_REG(hmdma->Instance->CBNDTR, MDMA_CBNDTR_BRC,
			((BlockCount - 1U) << MDMA_CBNDTR_BRC_Pos) & MDMA_CBNDTR_BRC);

	/* Clear all interrupt flags */
	__HAL_MDMA_CLEAR_FLAG(hmdma,
			MDMA_FLAG_TE | MDMA_FLAG_CTC | MDMA_CISR_BRTIF | MDMA_CISR_BTIF | MDMA_CISR_TCIF);
...
static void MDMA_Init(MDMA_HandleTypeDef *hmdma)
{
	...
	{
		/* Write new CBRUR Register value : source repeat block offset */
		hmdma->Instance->CBRUR =
				(((uint32_t) hmdma->Init.SourceBlockAddressOffset) & 0x0000FFFFU);
	}
	...
	else
	{
		/*write new CBRUR Register value : destination repeat block offset */
		hmdma->Instance->CBRUR |=
				((((uint32_t) hmdma->Init.DestBlockAddressOffset) & 0x0000FFFFU)
						<< MDMA_CBRUR_DUV_Pos);
	}
