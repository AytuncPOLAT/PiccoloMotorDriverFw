...
static HAL_StatusTypeDef
PCD_EP_OutXfrComplete_int(PCD_HandleTypeDef *hpcd, uint32_t epnum);
static HAL_StatusTypeDef
PCD_EP_OutSetupPacket_int(PCD_HandleTypeDef *hpcd, uint32_t epnum);
	...
	if (USB_GetMode(hpcd->Instance) == USB_OTG_MODE_DEVICE)
	{
		...
		if (__HAL_PCD_GET_FLAG(hpcd, USB_OTG_GINTSTS_LPMINT))
		{
			...
			if (hpcd->LPM_State == LPM_L0)
			{
				...
				hpcd->BESL = (hpcd->Instance->GLPMCFG & USB_OTG_GLPMCFG_BESL)
						>> 2U;
		...
		if (__HAL_PCD_GET_FLAG(hpcd, USB_OTG_GINTSTS_USBRST))
		{
			...
			(void) USB_EP0_OutStart(hpcd->Instance,
					(uint8_t) hpcd->Init.dma_enable, (uint8_t*) hpcd->Setup);
		...
		uint32_t epnum)
{
	...
	if (hpcd->Init.dma_enable == 1U)
	{
		...
		else if ((DoepintReg & (USB_OTG_DOEPINT_STUP | USB_OTG_DOEPINT_OTEPSPR))
				== 0U)
		{
			/* StupPktRcvd = 1 this is a setup packet */
			if ((gSNPSiD > USB_OTG_CORE_ID_300A)
