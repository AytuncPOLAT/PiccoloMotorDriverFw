 ...
 */
static void
FLASH_MassErase(uint32_t VoltageRange, uint32_t Banks);
static void
FLASH_OB_EnableWRP(uint32_t WRPSector, uint32_t Banks);
static void
FLASH_OB_DisableWRP(uint32_t WRPSector, uint32_t Bank);
static void
FLASH_OB_GetWRP(uint32_t *WRPState, uint32_t *WRPSector, uint32_t Bank);
static void
FLASH_OB_RDPConfig(uint32_t RDPLevel);
static uint32_t
FLASH_OB_GetRDP(void);
static void
FLASH_OB_PCROPConfig(uint32_t PCROConfigRDP, uint32_t PCROPStartAddr,
		uint32_t PCROPEndAddr, uint32_t Banks);
static void
FLASH_OB_GetPCROP(uint32_t *PCROPConfig, uint32_t *PCROPStartAddr,
		uint32_t *PCROPEndAddr, uint32_t Bank);
static void
FLASH_OB_BOR_LevelConfig(uint32_t Level);
static uint32_t
FLASH_OB_GetBOR(void);
static void
FLASH_OB_UserConfig(uint32_t UserType, uint32_t UserConfig);
static uint32_t
FLASH_OB_GetUser(void);
static void
FLASH_OB_BootAddConfig(uint32_t BootOption, uint32_t BootAddress0,
		uint32_t BootAddress1);
static void
FLASH_OB_GetBootAdd(uint32_t *BootAddress0, uint32_t *BootAddress1);
static void
FLASH_OB_SecureAreaConfig(uint32_t SecureAreaConfig,
		uint32_t SecureAreaStartAddr, uint32_t SecureAreaEndAddr,
		uint32_t Banks);
static void
FLASH_OB_GetSecureArea(uint32_t *SecureAreaConfig,
		uint32_t *SecureAreaStartAddr, uint32_t *SecureAreaEndAddr,
		uint32_t Bank);
static void
FLASH_CRC_AddSector(uint32_t Sector, uint32_t Bank);
static void
FLASH_CRC_SelectAddress(uint32_t CRCStartAddr, uint32_t CRCEndAddr,
		uint32_t Bank);
...
static void
FLASH_OB_SharedRAM_Config(uint32_t SharedRamConfig);
static uint32_t
FLASH_OB_SharedRAM_GetConfig(void);
#endif /* FLASH_OPTSR2_TCM_AXI_SHARED */

#if defined (FLASH_OPTSR2_CPUFREQ_BOOST)
static void
FLASH_OB_CPUFreq_BoostConfig(uint32_t FreqBoost);
static uint32_t
FLASH_OB_CPUFreq_GetBoost(void);
      ...
      if((pEraseInit->Banks & FLASH_BANK_2) == FLASH_BANK_2)
      {
		...
		else
		{
			...
			*SectorError = 0xFFFFFFFFU;
			...
			for (sector_index = pEraseInit->Sector;
					sector_index < (pEraseInit->NbSectors + pEraseInit->Sector);
					sector_index++)
			{
				...
				if ((pEraseInit->Banks & FLASH_BANK_1) == FLASH_BANK_1)
				{
					...
					status = FLASH_WaitForLastOperation(
							(uint32_t) FLASH_TIMEOUT_VALUE, FLASH_BANK_1);
...
HAL_StatusTypeDef HAL_FLASHEx_OBProgram(FLASH_OBProgramInitTypeDef *pOBInit)
{
	...
	/* Wait for last operation to be completed */
	if (FLASH_WaitForLastOperation((uint32_t) FLASH_TIMEOUT_VALUE, FLASH_BANK_1)
			!= HAL_OK)
	{
		status = HAL_ERROR;
	}
  ...
  else if(FLASH_WaitForLastOperation((uint32_t)FLASH_TIMEOUT_VALUE, FLASH_BANK_2) != HAL_OK)
  {
	...
	if (status == HAL_OK)
	{
		...
		if ((pOBInit->OptionType & OPTIONBYTE_PCROP) != 0U)
		{
			...
			FLASH_OB_PCROPConfig(pOBInit->PCROPConfig, pOBInit->PCROPStartAddr,
					pOBInit->PCROPEndAddr, pOBInit->Banks);
		}
    ...
    if((pOBInit->OptionType & OPTIONBYTE_CM4_BOOTADD) == OPTIONBYTE_CM4_BOOTADD)
    {
      FLASH_OB_CM4BootAddConfig(pOBInit->CM4BootConfig, pOBInit->CM4BootAddr0, pOBInit->CM4BootAddr1);
		...
		{
			FLASH_OB_BootAddConfig(pOBInit->BootConfig, pOBInit->BootAddr0,
					pOBInit->BootAddr1);
		}
#endif /*DUAL_CORE*/

		/* Secure area configuration */
		if ((pOBInit->OptionType & OPTIONBYTE_SECURE_AREA)
				== OPTIONBYTE_SECURE_AREA)
		{
			FLASH_OB_SecureAreaConfig(pOBInit->SecureAreaConfig,
					pOBInit->SecureAreaStartAddr, pOBInit->SecureAreaEndAddr,
					pOBInit->Banks);
		}
...
#if defined(FLASH_OPTSR2_TCM_AXI_SHARED)
		/* TCM / AXI Shared RAM configuration */
		if ((pOBInit->OptionType & OPTIONBYTE_SHARED_RAM)
...
#if defined(FLASH_OPTSR2_CPUFREQ_BOOST)
		/* CPU Frequency Boost configuration */
		if ((pOBInit->OptionType & OPTIONBYTE_FREQ_BOOST)
...
#endif /* DUAL_BANK */
	{
		pOBInit->OptionType |= (OPTIONBYTE_WRP | OPTIONBYTE_PCROP
				| OPTIONBYTE_SECURE_AREA);
		...
		FLASH_OB_GetSecureArea(&(pOBInit->SecureAreaConfig),
				&(pOBInit->SecureAreaStartAddr), &(pOBInit->SecureAreaEndAddr),
				pOBInit->Banks);
	}
		...
		uint32_t *CRC_Result)
{
	...
	if (status == HAL_OK)
	{
		if (pCRCInit->Bank == FLASH_BANK_1)
		{
			...
			FLASH->CRCCR1 |= FLASH_CRCCR_CLEAN_CRC | pCRCInit->BurstSize
					| pCRCInit->TypeCRC;
			...
			if (pCRCInit->TypeCRC == FLASH_CRC_SECTORS)
			{
				...
				/* Select CRC sectors */
				for (sector_index = pCRCInit->Sector;
						sector_index < (pCRCInit->NbSectors + pCRCInit->Sector);
						sector_index++)
				{
			...
			else
			{
				...
				FLASH_CRC_SelectAddress(pCRCInit->CRCStartAddr,
						pCRCInit->CRCEndAddr, FLASH_BANK_1);
			}
			...
			status = FLASH_CRC_WaitForLastOperation(
					(uint32_t) FLASH_TIMEOUT_VALUE, FLASH_BANK_1);
			...
			__HAL_FLASH_CLEAR_FLAG_BANK1(
					FLASH_FLAG_CRCEND_BANK1 | FLASH_FLAG_CRCRDERR_BANK1);
		}
#if defined (DUAL_BANK)
    else
    {
...
void FLASH_Erase_Sector(uint32_t Sector, uint32_t Banks, uint32_t VoltageRange)
{
  ...
  UNUSED(VoltageRange);
	...
	if ((Banks & FLASH_BANK_1) == FLASH_BANK_1)
	{
		...
		FLASH->CR1 |= (FLASH_CR_SER | VoltageRange
...
static void FLASH_OB_UserConfig(uint32_t UserType, uint32_t UserConfig)
{
  ...
  if((UserType & OB_USER_IWDG2_SW) != 0U)
  {
	...
	if ((UserType & OB_USER_NRST_STOP_D1) != 0U)
	{
		...
		assert_param(
				IS_OB_STOP_D1_RESET(UserConfig & FLASH_OPTSR_NRST_STOP_D1));
	...
	if ((UserType & OB_USER_NRST_STDBY_D1) != 0U)
	{
		...
		assert_param(
				IS_OB_STDBY_D1_RESET(UserConfig & FLASH_OPTSR_NRST_STBY_D1));
	...
	if ((UserType & OB_USER_IWDG_STOP) != 0U)
	{
		...
		assert_param(
				IS_OB_USER_IWDG_STOP(UserConfig & FLASH_OPTSR_FZ_IWDG_STOP));
	...
	if ((UserType & OB_USER_IWDG_STDBY) != 0U)
	{
		...
		assert_param(
				IS_OB_USER_IWDG_STDBY(UserConfig & FLASH_OPTSR_FZ_IWDG_SDBY));
	...
	if ((UserType & OB_USER_ST_RAM_SIZE) != 0U)
	{
		...
		assert_param(
				IS_OB_USER_ST_RAM_SIZE(UserConfig & FLASH_OPTSR_ST_RAM_SIZE));
  ...
  if((UserType & OB_USER_BCM7) != 0U)
  {
	...
	if ((UserType & OB_USER_NRST_STOP_D2) != 0U)
	{
		/* NRST_STOP option byte should be modified */
		assert_param(
				IS_OB_STOP_D2_RESET(UserConfig & FLASH_OPTSR_NRST_STOP_D2));
	...
	if ((UserType & OB_USER_NRST_STDBY_D2) != 0U)
	{
		/* NRST_STDBY option byte should be modified */
		assert_param(
				IS_OB_STDBY_D2_RESET(UserConfig & FLASH_OPTSR_NRST_STBY_D2));
 ...
 * @retval None
 */
static void FLASH_OB_PCROPConfig(uint32_t PCROPConfig, uint32_t PCROPStartAddr,
		uint32_t PCROPEndAddr, uint32_t Banks)
{
	/* Check the parameters */
	assert_param(IS_FLASH_BANK(Banks));
	...
	if ((Banks & FLASH_BANK_1) == FLASH_BANK_1)
	{
		...
		FLASH->PRAR_PRG1 = ((PCROPStartAddr - FLASH_BANK1_BASE) >> 8)
				| (((PCROPEndAddr - FLASH_BANK1_BASE) >> 8)
						<< FLASH_PRAR_PROT_AREA_END_Pos) | PCROPConfig;
	}
		...
		uint32_t *PCROPEndAddr, uint32_t Bank)
{
  ...
  if(Bank == FLASH_BANK_2)
  {
	...
	(*PCROPStartAddr) = ((regvalue & FLASH_PRAR_PROT_AREA_START) << 8)
			+ bankBase;
	(*PCROPEndAddr) = (regvalue & FLASH_PRAR_PROT_AREA_END)
 ...
 * @retval HAL Status
 */
static void FLASH_OB_BootAddConfig(uint32_t BootOption, uint32_t BootAddress0,
		uint32_t BootAddress1)
{
	/* Check the parameters */
		...
		uint32_t Banks)
{
	...
	if ((Banks & FLASH_BANK_1) == FLASH_BANK_1)
	{
		...
		FLASH->SCAR_PRG1 = ((SecureAreaStartAddr - FLASH_BANK1_BASE) >> 8)
				| (((SecureAreaEndAddr - FLASH_BANK1_BASE) >> 8)
		...
		uint32_t Bank)
{
  ...
  if(Bank == FLASH_BANK_2)
  {
	...
	(*SecureAreaStartAddr) = ((regvalue & FLASH_SCAR_SEC_AREA_START) << 8)
			+ bankBase;
	(*SecureAreaEndAddr) = (regvalue & FLASH_SCAR_SEC_AREA_END)
			>> FLASH_SCAR_SEC_AREA_END_Pos;
