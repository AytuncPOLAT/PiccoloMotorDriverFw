...
HAL_StatusTypeDef HAL_RCC_DeInit(void)
{
	...
	CLEAR_BIT(RCC->CR,
			RCC_CR_HSEON | RCC_CR_HSIKERON | RCC_CR_HSIDIV | RCC_CR_HSIDIVF | RCC_CR_CSION | RCC_CR_CSIKERON | RCC_CR_HSI48ON | RCC_CR_CSSHSEON);
	...
	RCC->PLLCKSELR = RCC_PLLCKSELR_DIVM1_5 | RCC_PLLCKSELR_DIVM2_5
		...
		RCC_OscInitTypeDef *RCC_OscInitStruct)
{
			...
			== RCC_OSCILLATORTYPE_HSE)
	{
		...
		if ((temp_sysclksrc == RCC_CFGR_SWS_HSE)
				|| ((temp_sysclksrc == RCC_CFGR_SWS_PLL1)
						&& ((temp_pllckselr & RCC_PLLCKSELR_PLLSRC)
		...
		else
		{
			...
			if (RCC_OscInitStruct->HSEState != RCC_HSE_OFF)
			{
				...
				{
					if ((uint32_t) (HAL_GetTick() - tickstart)
							> HSE_TIMEOUT_VALUE)
					{
						return HAL_TIMEOUT;
					}
			...
			== RCC_OSCILLATORTYPE_CSI)
	{
		...
		const uint32_t temp_sysclksrc = __HAL_RCC_GET_SYSCLK_SOURCE();
		const uint32_t temp_pllckselr = RCC->PLLCKSELR;
		if ((temp_sysclksrc == RCC_CFGR_SWS_CSI)
				|| ((temp_sysclksrc == RCC_CFGR_SWS_PLL1)
						&& ((temp_pllckselr & RCC_PLLCKSELR_PLLSRC)
	...
	if ((RCC_OscInitStruct->PLL.PLLState) != RCC_PLL_NONE)
	{
		...
		if (__HAL_RCC_GET_SYSCLK_SOURCE() != RCC_CFGR_SWS_PLL1)
		{
			if ((RCC_OscInitStruct->PLL.PLLState) == RCC_PLL_ON)
			{
				...
				assert_param(
						IS_RCC_PLLRGE_VALUE(RCC_OscInitStruct->PLL.PLLRGE));
				assert_param(
						IS_RCC_PLLVCO_VALUE(RCC_OscInitStruct->PLL.PLLVCOSEL));
				...
				assert_param(
						IS_RCC_PLLFRACN_VALUE(RCC_OscInitStruct->PLL.PLLFRACN));
		...
		else
		{
			...
			if (((RCC_OscInitStruct->PLL.PLLState) == RCC_PLL_OFF)
					|| (READ_BIT(temp1_pllckcfg, RCC_PLLCKSELR_PLLSRC)
							!= RCC_OscInitStruct->PLL.PLLSource)
					|| ((READ_BIT(temp1_pllckcfg, RCC_PLLCKSELR_DIVM1)
							>> RCC_PLLCKSELR_DIVM1_Pos)
							!= RCC_OscInitStruct->PLL.PLLM)
					|| (READ_BIT(temp2_pllckcfg, RCC_PLL1DIVR_N1)
							!= (RCC_OscInitStruct->PLL.PLLN - 1U))
					|| ((READ_BIT(temp2_pllckcfg, RCC_PLL1DIVR_P1)
							>> RCC_PLL1DIVR_P1_Pos)
							!= (RCC_OscInitStruct->PLL.PLLP - 1U))
					|| ((READ_BIT(temp2_pllckcfg, RCC_PLL1DIVR_Q1)
							>> RCC_PLL1DIVR_Q1_Pos)
							!= (RCC_OscInitStruct->PLL.PLLQ - 1U))
					|| ((READ_BIT(temp2_pllckcfg, RCC_PLL1DIVR_R1)
							>> RCC_PLL1DIVR_R1_Pos)
							!= (RCC_OscInitStruct->PLL.PLLR - 1U)))
			{
				return HAL_ERROR;
			}
			else
			{
				...
				temp1_pllckcfg = ((RCC->PLL1FRACR & RCC_PLL1FRACR_FRACN1)
		...
		const RCC_ClkInitTypeDef *RCC_ClkInitStruct, uint32_t FLatency)
{
	...
	if (((RCC_ClkInitStruct->ClockType) & RCC_CLOCKTYPE_D1PCLK1)
			== RCC_CLOCKTYPE_D1PCLK1)
	{
#if defined (RCC_D1CFGR_D1PPRE)
		if ((RCC_ClkInitStruct->APB3CLKDivider)
			...
			== RCC_CLOCKTYPE_PCLK1)
	{
		...
		if ((RCC_ClkInitStruct->APB1CLKDivider)
			...
			== RCC_CLOCKTYPE_PCLK2)
	{
		...
		if ((RCC_ClkInitStruct->APB2CLKDivider)
				> (RCC->D2CFGR & RCC_D2CFGR_D2PPRE2))
		{
			assert_param(IS_RCC_PCLK2(RCC_ClkInitStruct->APB2CLKDivider));
			MODIFY_REG(RCC->D2CFGR, RCC_D2CFGR_D2PPRE2,
	...
	if (((RCC_ClkInitStruct->ClockType) & RCC_CLOCKTYPE_D3PCLK1)
			...
			== RCC_CLOCKTYPE_HCLK)
	{
				...
				> (RCC->D1CFGR & RCC_D1CFGR_HPRE))
		{
			...
			MODIFY_REG(RCC->D1CFGR, RCC_D1CFGR_HPRE,
			...
			== RCC_CLOCKTYPE_SYSCLK)
	{
		...
		MODIFY_REG(RCC->D1CFGR, RCC_D1CFGR_D1CPRE,
	...
	case RCC_CFGR_SWS_PLL1: /* PLL1 used as system clock  source */
		...
		if (pllm != 0U)
		{
			...
			case RCC_PLLSOURCE_HSI: /* HSI used as PLL clock source */
				...
				{
					hsivalue =
							(HSI_VALUE >> (__HAL_RCC_GET_HSI_DIVIDER() >> 3));
					pllvco =
...
void HAL_RCC_GetOscConfig(RCC_OscInitTypeDef *RCC_OscInitStruct)
{
...
#else
	RCC_OscInitStruct->CSICalibrationValue =
			(uint32_t) (READ_BIT(RCC->CSICFGR, RCC_CSICFGR_CSITRIM)
...
#else
	RCC_OscInitStruct->HSICalibrationValue =
			(uint32_t) (READ_BIT(RCC->HSICFGR, RCC_HSICFGR_HSITRIM)
  ...
  else
  {
	...
	RCC_OscInitStruct->PLL.PLLSource = (uint32_t) (RCC->PLLCKSELR
			& RCC_PLLCKSELR_PLLSRC);
	RCC_OscInitStruct->PLL.PLLM = (uint32_t) ((RCC->PLLCKSELR
			& RCC_PLLCKSELR_DIVM1) >> RCC_PLLCKSELR_DIVM1_Pos);
	RCC_OscInitStruct->PLL.PLLN = (uint32_t) ((RCC->PLL1DIVR & RCC_PLL1DIVR_N1)
			>> RCC_PLL1DIVR_N1_Pos) + 1U;
	RCC_OscInitStruct->PLL.PLLR = (uint32_t) ((RCC->PLL1DIVR & RCC_PLL1DIVR_R1)
			>> RCC_PLL1DIVR_R1_Pos) + 1U;
	RCC_OscInitStruct->PLL.PLLP = (uint32_t) ((RCC->PLL1DIVR & RCC_PLL1DIVR_P1)
			>> RCC_PLL1DIVR_P1_Pos) + 1U;
	RCC_OscInitStruct->PLL.PLLQ = (uint32_t) ((RCC->PLL1DIVR & RCC_PLL1DIVR_Q1)
			>> RCC_PLL1DIVR_Q1_Pos) + 1U;
	RCC_OscInitStruct->PLL.PLLRGE = (uint32_t) ((RCC->PLLCFGR
			& RCC_PLLCFGR_PLL1RGE));
	RCC_OscInitStruct->PLL.PLLVCOSEL = (uint32_t) ((RCC->PLLCFGR
			& RCC_PLLCFGR_PLL1VCOSEL) >> RCC_PLLCFGR_PLL1VCOSEL_Pos);
	RCC_OscInitStruct->PLL.PLLFRACN = (uint32_t) (((RCC->PLL1FRACR
			& RCC_PLL1FRACR_FRACN1) >> RCC_PLL1FRACR_FRACN1_Pos));
}
...
#if defined(RCC_D1CFGR_D1CPRE)
	/* Get the SYSCLK configuration ----------------------------------------------*/
	RCC_ClkInitStruct->SYSCLKDivider = (uint32_t) (RCC->D1CFGR
			& RCC_D1CFGR_D1CPRE);

	/* Get the D1HCLK configuration ----------------------------------------------*/
	RCC_ClkInitStruct->AHBCLKDivider =
			(uint32_t) (RCC->D1CFGR & RCC_D1CFGR_HPRE);

	/* Get the APB3 configuration ----------------------------------------------*/
	RCC_ClkInitStruct->APB3CLKDivider = (uint32_t) (RCC->D1CFGR
			& RCC_D1CFGR_D1PPRE);

	/* Get the APB1 configuration ----------------------------------------------*/
	RCC_ClkInitStruct->APB1CLKDivider = (uint32_t) (RCC->D2CFGR
			& RCC_D2CFGR_D2PPRE1);

	/* Get the APB2 configuration ----------------------------------------------*/
	RCC_ClkInitStruct->APB2CLKDivider = (uint32_t) (RCC->D2CFGR
			& RCC_D2CFGR_D2PPRE2);

	/* Get the APB4 configuration ----------------------------------------------*/
	RCC_ClkInitStruct->APB4CLKDivider = (uint32_t) (RCC->D3CFGR
			& RCC_D3CFGR_D3PPRE);
#else
  /* Get the SYSCLK configuration ----------------------------------------------*/
