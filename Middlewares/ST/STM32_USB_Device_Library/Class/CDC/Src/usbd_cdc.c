...
NULL,
  ...
  NULL,
#else
		USBD_CDC_GetHSCfgDesc, USBD_CDC_GetFSCfgDesc,
...
static uint8_t USBD_CDC_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx)
{
	...
	hcdc = (USBD_CDC_HandleTypeDef*) USBD_malloc (
...
static uint8_t USBD_CDC_DataOut(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
	...
	((USBD_CDC_ItfTypeDef*) pdev->pUserData[pdev->classId])->Receive(
			hcdc->RxBuffer, &hcdc->RxLength);
...
static uint8_t USBD_CDC_EP0_RxReady(USBD_HandleTypeDef *pdev)
{
	...
	{
		((USBD_CDC_ItfTypeDef*) pdev->pUserData[pdev->classId])->Control(
				hcdc->CmdOpCode, (uint8_t*) hcdc->data,
...
USBD_CDC_GetFSCfgDesc(uint16_t *length)
{
	...
	CDC_CMD_EP);
	USBD_EpDescTypeDef *pEpOutDesc = USBD_GetEpDesc(USBD_CDC_CfgDesc,
	CDC_OUT_EP);
