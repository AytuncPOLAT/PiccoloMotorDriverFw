...
typedef struct _USBD_CDC_Itf
{
	...
	(*DeInit)(void);
	int8_t
	(*Control)(uint8_t cmd, uint8_t *pbuf, uint16_t length);
	int8_t
	(*Receive)(uint8_t *Buf, uint32_t *Len);
	int8_t
	(*TransmitCplt)(uint8_t *Buf, uint32_t *Len, uint8_t epnum);
} USBD_CDC_ItfTypeDef;
...
#else
uint8_t
USBD_CDC_SetTxBuffer(USBD_HandleTypeDef *pdev, uint8_t *pbuff, uint32_t length);
uint8_t
